...
/// each release forces a re-parse of already cached specs — even when the
/// upstream HTML is byte-for-byte unchanged. Bumping the crate version in
/// `Cargo.toml` (which every release already does) is therefore enough to
/// invalidate stale indexes; there is no separate constant to remember to bump.
