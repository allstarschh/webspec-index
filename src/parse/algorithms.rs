...
/// Render a *source-form* `<emu-alg>` — ecmarkup markdown steps (`1. …`,
/// nested by indentation) that have not been built into an `<ol>`. ECMA-262's
/// committed `spec.html` stores algorithms this way (the `<ol>` is generated by
/// ecmarkup at build time), so `render_algorithm_ol` finds no list. We keep the
/// steps as text — dedented to the shallowest step so relative nesting is
/// preserved — so they still appear in section content and PR diffs. The
/// ecmarkup markers (`_var_`, `*value*`) double as markdown emphasis, so the
/// output reads reasonably as-is.
pub fn render_algorithm_source(emu_alg: &ElementRef) -> String {
    let text: String = emu_alg.text().collect();
    dedent_block(&text)
}

/// Strip the common leading indentation from a block of text, preserving
/// relative indentation between lines and trimming surrounding blank lines.
...
mod tests {
    ...
    #[test]
    fn test_render_algorithm_source() {
        // Source-form emu-alg (ECMA-262 spec.html): auto-numbered `1.` text with
        // indented nesting, no built <ol>.
        let html = r#"
            <emu-alg>
        ...
        let emu_alg = fragment.select(&selector).next().unwrap();

        let result = render_algorithm_source(&emu_alg);
        let expected = "1. Let _ctor_ be the *this* value.\n\
                        1. If _status_ is an abrupt completion, then\n  \
                        1. Perform ? Call(_x_).\n\
                        1. Return _ctor_.";
